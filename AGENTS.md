...
- Agent runtime service: lives in the private repo `qaml-ai/agent-runtime` (hosted at agents.camelai.dev). Chiridion consumes only its TypeScript SDK, `@qaml-ai/agent-runtime` from GitHub Packages (`.npmrc` scopes `@qaml-ai` there and reads `NODE_AUTH_TOKEN`), in `workers/main/src/chat-thread/service-agent.ts`. `bun run dev:external-agent` and `bun run test:agent-service-adapter` start a runtime checkout from `AGENT_RUNTIME_DIR` (default `../agent-runtime`). Codemode source preparation lives in `packages/agent-core/`.
