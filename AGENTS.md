...
- The browser receives canonical `Message[]` snapshots with coarse queued/running/terminal state and final text, not token-level replay or internal tool traces. The old Pi-chunk/UIMessage conversion and steering split are gone. Active-turn steering, crash resume/salvage ladders, long-lived Pi sessions, nested agents, and model-blocking user-question/connection-setup tools are intentionally unavailable. Future versions of those features require their own finite durable state, deadline, and formal action.
