...
- Agent runtime service: lives in the private repo `qaml-ai/agent-runtime` (hosted at agents.camelai.dev). Chiridion consumes only its TypeScript SDK, `@camelai/agent-runtime` from public npm, in `workers/main/src/chat-thread/service-agent.ts`. `bun run dev:external-agent` and `bun run test:agent-service-adapter` start a runtime checkout from `AGENT_RUNTIME_DIR` (default `../agent-runtime`) and need `AGENT_DATABASE_URL` (the runtime keeps its control plane in Postgres). Codemode source preparation lives in `packages/agent-core/`.
