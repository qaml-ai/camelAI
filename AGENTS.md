...
Docker (v28.5.2 + compose plugin) is preinstalled in the snapshot with the DinD-compatible config (`fuse-overlayfs` storage driver, `iptables-legacy`), and the `ubuntu` user is in the `docker` group. The daemon is **not** auto-started (no systemd here), so start it once per session and it stays up:
...
- Docker is intentionally **not** in the environment update script (it is a system dependency baked into the snapshot, and daemon startup is a service action). Keep the update script minimal (`bun install --frozen-lockfile`).
