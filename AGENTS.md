...
- **How persistence works here (read first).** Tools installed during environment setup — Bun (`~/.bun`), Docker (apt), Playwright browsers, and the generated `.dev.vars` — persist to future agents **only if a base environment snapshot was saved** (an explicit action in the Cloud environment settings) or they are baked into a `.cursor/environment.json` Dockerfile/`install` command. The `install` command (`bun install --frozen-lockfile`) alone does **not** recreate them — and it even assumes Bun already exists. If a future agent boots the default base image with no snapshot, expect to reinstall Bun/Docker/Playwright and recreate `.dev.vars`. Daemons (e.g. `dockerd`) are never part of a snapshot and must be (re)started each run.
...
- **`.dev.vars` is required to boot** and must contain non-empty `TOKEN_SIGNING_SECRET` and `INTEGRATION_SECRET_KEY` (random values are fine — copy `.dev.vars.example`). It is gitignored; the setup session created one, which persists to future agents only if the snapshot was saved (see persistence note above) — otherwise recreate it by copying `.dev.vars.example` and filling the two secrets.
- **Exercising a real chat turn offline (no model creds):** run the deterministic fake LLM `node scripts/fake-llm.mjs` (port `8788`) and start the dev server with `TEST_LLM_REPLAY_URL=http://localhost:8788` so `resolvePiModel` routes model calls to it. The fake echoes text after `Reply with:` (prefixed `[fake-llm] `). Full hello-world command:
  ...
  ```
- **Local Playwright E2E:** `E2E_LOCAL=1 bun run test:e2e` reuses an already-running `:3001` dev server (and `:8788` fake LLM) via `reuseExistingServer`; if you want Playwright to own both, stop your manual servers first. Chromium + system deps were installed during setup (present in future agents only if the snapshot was saved; otherwise run `bunx playwright install --with-deps chromium`). The two `connections-local` specs can flake against a live Vite dev server (dialog open timing); the app page itself loads fine.
...
- **Bun** was installed at `~/.bun/bin/bun` and symlinked to `/usr/local/bin/bun` so it resolves in non-login shells (the `install` command's `bun install --frozen-lockfile` depends on Bun already being present — so Bun must live in the saved snapshot or the environment Dockerfile, not just in the install command). If Bun is missing on a fresh VM, reinstall with `curl -fsSL https://bun.sh/install | bash`.

### Docker & self-hosting in the Cloud VM

Docker (v28.5.2 + compose plugin) was installed during setup with the DinD-compatible config (`fuse-overlayfs` storage driver, `iptables-legacy`), and the `ubuntu` user was added to the `docker` group. This persists to future agents only if the snapshot was saved or Docker is baked into the environment Dockerfile (see the persistence note at the top of this section) — otherwise reinstall it. The daemon is **not** auto-started (no systemd here), so start it each session. The durable way is the environment `start` command in `.cursor/environment.json` (`sudo service docker start`); manually you can run:

```bash
sudo dockerd >/tmp/dockerd.log 2>&1 &   # then: docker info
```

- Self-host lives in `SELF_HOSTING.md` / `infra/selfhost/README.md`; scripts are `bun run selfhost:*`. `selfhost:init` writes a gitignored `.env.selfhost` (defaults to `SELFHOST_DEPLOYMENT_MODE=source`). These CI-parity checks run fully offline: `bun run selfhost:init`, `docker compose --env-file .env.selfhost -f docker-compose.selfhost.yml config --quiet`, and `bun run test:selfhost`.
- The default init stack (`bundled-pomerium` + `automatic` TLS) needs real OIDC + DNS credentials. For a localhost-only browser smoke, patch `.env.selfhost` to `SELFHOST_AUTH_MODE=local` + `LOCAL_AUTH_BYPASS=1` and `SELFHOST_TLS_MODE=external` (plain HTTP on `127.0.0.1:8080`); a real `SELFHOST_AI_API_KEY` is only needed for actual chat.
- **Hard limitation — Docker-sandbox agent features cannot run in this microVM kernel.** `workerd`'s `localDocker` engine (project builds, notebook analysis, SQL — and therefore the `selfhost:container:smoke:*` checks and the sandbox features of a full `selfhost:up`) starts an egress-interceptor sidecar that requires the netfilter **TPROXY** target. This Firecracker kernel has no loadable modules (no `modprobe`, no `/lib/modules`) and no TPROXY, so the sidecar aborts with `Extension TPROXY revision 0 not supported, missing kernel module?` and the sandbox container never becomes reachable. Docker itself, image builds, the self-host web app, and the offline CI-parity checks above all work; only the in-VM Docker-sandbox execution is blocked by the kernel. Do not spend time trying to `modprobe`/patch iptables around this — it is a kernel capability gap, not a package/config issue.
- Docker is intentionally **not** in the environment `install` command: as a system dependency it belongs in the base snapshot or the environment Dockerfile, and starting the daemon belongs in the `start` command — not the (idempotent, per-boot) install script. Keep the install command minimal (`bun install --frozen-lockfile`).
