...
import { normalizeAutomationOutcome } from "./automation-turn-report";
import {
...
export class ChatThreadRuntimeDO extends DurableObject<ChatEnv> {
  ...
  protected createDriver(): DurableTurnDriver {
    return new DurableTurnDriver({
        ...
        return createBoundedTurnRunner(
          this.captureEvalTools(
            createPiTurnAdapter({
              ...
              env: this.env,
              store: this.store,
              automationOutcome: context.turn.automationRun?.requiresExplicitOutcome
                ? {
                    execute: async (args, signal) => {
                      if (signal.aborted) throw signal.reason;
                      if (this.store.recordedAutomationOutcome(context.turn.id))
                        throw new Error("Automation outcome was already reported for this run");
                      const outcome = normalizeAutomationOutcome(args);
                      if (!outcome)
                        throw new Error("A valid automation status and summary are required");
                      return { content: [{ type: "text", text:
                        `Automation outcome recorded: ${outcome.status}` }], details: outcome };
                    },
                  }
                : undefined,
            }),
      ...
      reportAutomation: async (report, signal) => {
        const cron = this.env.WORKSPACE_CRON;
        if (!cron || signal.aborted) return false;
        return cron.get(cron.idFromName(report.workspaceId))
          .recordScheduledPromptRunResult({ workspaceId: report.workspaceId,
            promptId: report.automationId, runId: report.runId,
    ...
    const result = this.store.admit(
      {
        ...
        userContent: message,
        userDisplay: message,
        automationRun: body.automationRun,
      },
      now,
    );
    if (!result.ok) {
  ...
  getRuntimeStatus(): ChatThreadRuntimeStatus {
    ...
    return {
      isStreaming: this.store.hasPendingTurn(),
