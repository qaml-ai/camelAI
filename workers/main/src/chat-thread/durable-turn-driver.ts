...
export class DurableTurnDriver {
  ...
  private async armNext(result: StoreResult): Promise<void> {
    const now = this.now();
    const storeAt = result.shouldArmAlarm
      ? this.options.store.nextAlarmAt(now)
      : null;
