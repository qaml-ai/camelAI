...
export interface DurableChatTurn extends AdmitChatTurn {
...
export class DurableChatTurnStore {
  ...
  private pruneHistory(): void {
    this.sql.exec(
      `UPDATE chat_turns_v2 SET retained = 0, user_id = NULL,
         ...
         , checkpoint_json = ?
       WHERE retained = 1
         AND status IN ('completed', 'failed', 'interrupted') AND id NOT IN (
         SELECT id FROM (
           SELECT id,
  ...
  private terminal(
    ...
    return this.storage.transactionSync(() => {
      ...
      const column =
        status === "completed" ? "assistant_final" : "assistant_error";
      const assistantRenderJson = terminalRenderJson(
        running.checkpoint_json,
        output,
        status === "completed",
      );
      const terminal = this.sql
        .exec<TurnRow>(
          `UPDATE chat_turns_v2 SET status = ?, ${column} = ?,
          assistant_render_json = ?, lease_expires_at = NULL,
      ...
      const revision = this.bumpRevision();
      this.appendEvent(
        status === "completed" ? "CompleteTurn" : "FailTurn",
        id,
        status,
        revision,
        at,
  ...
  expire(now: number): StoreResult {
    return this.storage.transactionSync(() => {
      ...
      if (turn.status === "queued") {
        this.sql.exec(
          `UPDATE chat_turns_v2 SET status = 'failed', assistant_error = ?,
        ...
        const revision = this.bumpRevision();
        this.appendEvent("ExpireOperation", turn.id, "failed", revision, at);
        const terminal = this.find(turn.id) as TurnRow;
        const result = this.accepted(terminal);
        this.pruneHistory();
        return result;
      }
  ...
  *readModelContext(excludeTurnId: string): Iterable<ChatModelContextTurn> {
  ...
  private interruptRunning(
    ...
    const revision = this.bumpRevision();
    this.appendEvent(event, turn.id, "interrupted", revision, at);
    const terminal = this.find(turn.id) as TurnRow;
    const result = this.accepted(terminal);
    this.pruneHistory();
    return result;
  ...
  shouldArmAlarm(): boolean {
    return this.nextAlarmAt(0) !== null;
  }
