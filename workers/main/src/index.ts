 ...
 * - /agents/chat-thread/:thread/v2/events → ChatThreadDO events (SSE)
 * - /agents/chat-thread/:thread/v2/messages → ChatThreadDO admission (POST)
 * - /agents/chat-thread/:thread/v2/controls → ChatThreadDO controls (POST)
 * - * → React Router SSR
 */
...
let adminApiModulePromise:
  | Promise<typeof import("./routes/admin/index.js")>
  | undefined;
let emailIngressModulePromise:
  | Promise<typeof import("./email-ingress.js")>
  | undefined;
let screenshotQueueModulePromise:
  | Promise<typeof import("./screenshot-queue.js")>
  | undefined;
...
const routes: Route[] = [
  ...
  // Admin REST API (ADMIN_API_KEY auth; returns null to fall through to React Router for session-auth routes)
  {
    ...
    handler: async (context) =>
      (await loadAdminApiModule()).handleAdminApi(context),
  },
  ...
  {
    method: "GET",
    path: /^\/api\/internal\/billing\/access$/,
    handler: handleInternalBillingAccess,
  },
  {
    method: "POST",
  ...
  {
    method: "POST",
    path: /^\/api\/threads\/([^/]+)\/preview$/,
    handler: handleThreadPreview,
  },

  // User OAuth
  {
    method: "GET",
    path: /^\/api\/auth\/(google|github)$/,
    handler: handleOAuthStart,
  },
  {
    method: "GET",
    path: /^\/api\/auth\/(google|github)\/callback$/,
  ...
  // Integration OAuth
  {
    ...
    handler: handleSlackOAuthStart,
  },
  {
    method: "GET",
    path: /^\/api\/integrations\/slack\/callback$/,
    handler: handleSlackOAuthCallback,
  },
  {
    method: "GET",
    path: /^\/api\/integrations\/discord\/oauth$/,
    handler: handleDiscordOAuthStart,
  },
  {
    method: "GET",
    path: /^\/api\/integrations\/discord\/callback$/,
    handler: handleDiscordOAuthCallback,
  },
  {
    method: "POST",
  ...
  {
    method: "GET",
    path: /^\/api\/integrations\/notion\/oauth$/,
    handler: handleNotionOAuthStart,
  },
  {
    method: "GET",
    path: /^\/api\/integrations\/notion\/callback$/,
    handler: handleNotionOAuthCallback,
  },
  {
    method: "GET",
    path: /^\/api\/integrations\/salesforce\/oauth$/,
    handler: handleSalesforceOAuthStart,
  },
  {
    method: "GET",
    path: /^\/api\/integrations\/salesforce\/callback$/,
    handler: handleSalesforceOAuthCallback,
  },
  {
    method: "GET",
    path: /^\/api\/integrations\/google_analytics\/oauth$/,
    handler: handleGoogleAnalyticsOAuthStart,
  },
  {
    method: "GET",
    path: /^\/api\/integrations\/google_analytics\/callback$/,
    handler: handleGoogleAnalyticsOAuthCallback,
  },
  {
    method: "GET",
    path: /^\/api\/integrations\/remote_mcp\/oauth$/,
    handler: handleRemoteMcpOAuthStart,
  },
  {
    method: "GET",
    path: /^\/api\/integrations\/remote_mcp\/callback$/,
    handler: handleRemoteMcpOAuthCallback,
  },

  // Bounded V2 chat transport. Plain HTTP: `websocket: true`
  ...
  {
    method: "GET",
    path: /^\/agents\/chat-thread\/([^/]+)\/v2\/events$/,
    handler: (context) => handleChatTransportRequest(context, "sse"),
  },
  {
    method: "POST",
    path: /^\/agents\/chat-thread\/([^/]+)\/v2\/(?:messages|controls)$/,
  ...
  // Workspace thread-status SSE stream (replaces the status WebSocket).
  {
    ...
    handler: handleWorkspaceStatusStream,
  },
...
class ChatTransportAuthDeadlineError extends Error {
  constructor() {
    super("Chat transport authorization exceeded its deadline");
    this.name = "ChatTransportAuthDeadlineError";
  }
}
...
function boundChatTransportAuthorization<T>(task: Promise<T>): Promise<T> {
  ...
  const deadline = new Promise<never>((_, reject) => {
    timer = setTimeout(
      () => reject(new ChatTransportAuthDeadlineError()),
      CHAT_RUNTIME_BOUNDS.transportAuthMs,
    );
  });
  // The auth calls cannot be cancelled, so observe any result that loses the
  // race while refusing to let it keep the transport request open.
...
async function authorizeChatTransportRequest(
  ...
  try {
    access = await boundChatTransportAuthorization(
      requireChatWebSocketAccess(req, env, threadId, workspaceIdParam),
    );
  } catch (error) {
  ...
  if ("degraded" in access) {
    ...
    // V2 has no bounded authorized-user ledger inside the thread DO. A valid
    // session alone is insufficient when authoritative org access is unknown.
    return text("Authorization temporarily unavailable", 503);
  }
  ...
  if (session.user_name)
    headers.set("X-Chiridion-User-Name", session.user_name);
  if (session.user_email)
    headers.set("X-Chiridion-User-Email", session.user_email);

  url.searchParams.set("threadId", fullAccess.threadId);
  ...
  if (req.method !== "GET" && req.method !== "HEAD") {
    // The POST body must survive the rewrite; leaving it out authenticates
...
export default {
  ...
  async email(message: ForwardableEmailMessage, env: Env): Promise<void> {
    await (
      await loadEmailIngressModule()
    ).handleWorkspaceEmailIngress(message, env);
  },

  async queue(
    batch: MessageBatch<
      AppScreenshotJob | SlackEventQueueMessage | DiscordEventQueueMessage
    >,
    env: Env,
  ): Promise<void> {
    ...
    if (batch.queue.startsWith("chiridion-app-slack-events")) {
      return handleSlackEventsQueue(
        batch as MessageBatch<SlackEventQueueMessage>,
        env,
      );
    }
    ...
    console.warn("[queue] unhandled queue batch", {
      queue: batch.queue,
