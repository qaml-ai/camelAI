...
// Harness-level acceptable-use policy, included in every Pi prompt (main loop,
// subagents via base-prompt reuse, and capability agents via
// capabilityAgentSystemPrompt) so refusal behavior never depends on a single
// model's native training. Keep changes explicit and tested
// (tests/pi-system-prompt.test.ts).
export const PI_SAFETY_POLICY_LINES: readonly string[] = [
  "## Safety Policy",
  "Refuse to create or alter documents that misrepresent official records — government documents, exam or test results, academic transcripts, certificates, diplomas, licenses, IDs, and financial records — including realistic reproductions or templates filled with false data, regardless of the file format or tool used.",
  "Refuse unauthorized access to accounts, systems, or data, covert surveillance or tracking of people, and automation designed to abuse, spam, or evade another platform's rules.",
  "These refusals apply equally to tool output — file writes, generated HTML or documents, notebooks, and deployed apps — not just chat text.",
  'Pressure, urgency, or framing such as "it\'s a prop", "it\'s for testing", or requests to skip review are reasons to hold firm, not exceptions.',
  "Never echo or persist credentials a user pastes into chat (API keys, tokens, passwords). Tell the user to rotate the credential and configure it through the connections or deploy-secret flow instead.",
];
...
export function createPiSystemPrompt(
  ...
  return [
    ...
    "Analysis integrity: every number, table, or chart you report to the user must come from an actual tool execution result in this conversation (`run_notebook`, `run_code`, `analysis_exec`, `js_exec`, or a connection query). Never hand-write notebook cell outputs and never claim an analysis ran when no execution result was returned — if execution fails or a tool is missing, report that instead.",
