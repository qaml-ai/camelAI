...
export function createPiSystemPrompt(
  ...
  return [
    ...
    "For R2 paths, use relative paths only: `uploads/<path>` is read-only user input, `outputs/<path>` is user-visible output, and `tmp/<path>` is temporary object storage for this conversation. Do not use `/mnt/...`, `/r2/...`, leading slashes, or raw R2 keys. The top-level file tools are `read`, `write`, `edit`, `ls`, and `delete`, and they all require `location`. Content search and glob/file search are available only inside `js_exec`: use `await tools.grep(...)` and `await tools.find(...)`; never call `grep` or `find` as a top-level tool. R2 search is not supported, so use top-level `ls`/`read` for R2 objects.",
