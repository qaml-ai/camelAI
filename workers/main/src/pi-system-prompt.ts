...
export type PiSystemPromptOptions = {
  ...
  /**
   * When false, deployed apps on this deployment do not get a working
   * `env.CONNECTIONS` broker. Chat/`js_exec` connections remain available.
   * Defaults to true.
   */
  deployedConnectionsBindingEnabled?: boolean;
  /**
   * When true, deployed apps are always protected by the self-host ingress SSO
   * policy and the agent must not offer per-app public/private controls.
   */
  selfhostAppAccessSsoOnly?: boolean;
  ...
  promptAppend?: string;
  /** Reject before joining once the complete UTF-8 prompt would exceed this. */
  maxBytes?: number;
};

/** Agent-facing caveat when CONNECTIONS_BINDING_ENABLED is false. */
export const DEPLOYED_CONNECTIONS_BINDING_DISABLED_PROMPT =
  "Deployed-app CONNECTIONS binding is disabled on this deployment: published apps do not receive `env.CONNECTIONS` / `context.cloudflare.env.CONNECTIONS`, and any leftover binding fails closed. Do not build, scaffold, or deploy apps that call workspace connections from app code. Prefer `js_exec` (and chat connection tools) for connection work. Skip the `integration-dashboard` template unless the UI is purely local/mock and never calls CONNECTIONS. Skills that mention deployed-app CONNECTIONS are overridden by this rule.";

/** Agent-facing app-access policy for self-host deployments. */
export const SELFHOST_APP_ACCESS_SSO_PROMPT =
  "Every deployed app on this self-host installation is protected by the configured Pomerium or Cloudflare Access SSO policy. There is no per-app public/private setting, and `set_app_visibility` is unavailable. Do not offer or attempt app visibility changes. Treat every app URL as private to authenticated enterprise users; if an embedded preview cannot complete the SSO flow, ask the user to open the app URL in a top-level browser tab.";

export type PiSubagentMode = "agent" | "explore";

function utf8ByteLengthWithin(value: string, limit: number): number | null {
...
function boundedPromptValue(
  ...
  if (
    maxBytes !== undefined &&
    utf8ByteLengthWithin(value, maxBytes) === null
  ) {
    throw new Error(`${label} exceeds the system-prompt byte limit`);
  }
  return value;
}

function formatSkillHint(
  description: string | undefined,
  maxBytes?: number,
): string | undefined {
  if (!description) return undefined;
  boundedPromptValue(description, maxBytes, "Skill description");
  ...
  const hint =
    useWhenIndex >= 0
      ? normalized
          .slice(useWhenIndex)
          .replace(/^Use (?:this skill )?when/i, "Use when")
      : normalized;
...
export function createPiSystemPrompt(
  ...
  const lines: string[] = [];
  let bytes = 0;
  const add = (line: string | null | undefined): void => {
    if (!line) return;
    if (options.maxBytes !== undefined) {
      const separator = lines.length ? 1 : 0;
      const lineBytes = utf8ByteLengthWithin(
        line,
        options.maxBytes - bytes - separator,
      );
      if (lineBytes === null) {
        throw new Error("System prompt exceeds its runtime byte limit");
      }
      bytes += separator + lineBytes;
    }
    lines.push(line);
  };
  const webResearchGuidance =
    options.researchAvailable === false
  ...
  const deployedConnectionsEnabled =
    options.deployedConnectionsBindingEnabled !== false;
  const selfhostAppAccessSsoOnly = options.selfhostAppAccessSsoOnly === true;
  const integrationDashboardTemplate = deployedConnectionsEnabled
    ? "`integration-dashboard` for apps centered on workspace connections"
    : "`integration-dashboard` only when the UI is local/mock and never calls CONNECTIONS (deployed-app CONNECTIONS is disabled here)";
  const promptPrepend = options.promptPrepend
    ? boundedPromptValue(
        options.promptPrepend,
        options.maxBytes,
        "Deployment prompt prepend",
      ).trim()
    : undefined;
  const promptAppend = options.promptAppend
    ? boundedPromptValue(
        options.promptAppend,
        options.maxBytes,
        "Deployment prompt append",
      ).trim()
    : undefined;
  const workspaceId = boundedPromptValue(
    context.workspaceId,
    options.maxBytes,
    "Workspace id",
  );
  const threadId = boundedPromptValue(
    context.threadId,
    options.maxBytes,
    "Thread id",
  );
  const orgId = boundedPromptValue(
    context.orgId,
    options.maxBytes,
    "Organization id",
  );
  ...
  for (const line of [
    ...
    selfhostAppAccessSsoOnly ? SELFHOST_APP_ACCESS_SSO_PROMPT : null,
  ...
  for (const name of options.skillNames) {
    boundedPromptValue(name, options.maxBytes, "Skill name");
    const descriptions = options.skillDescriptions;
    const descriptor = descriptions
      ? Object.getOwnPropertyDescriptor(descriptions, name)
      : undefined;
    if (descriptor && !("value" in descriptor)) {
      throw new Error("Skill description contains an accessor");
    }
    const description = descriptor?.value;
    if (description !== undefined && typeof description !== "string") {
      throw new Error("Skill description is not a string");
    }
    const hint = formatSkillHint(description, options.maxBytes);
    const instruction = `call \`read_skill({ skill: "${name}" })\``;
    add(
      hint
        ? `- ${name}: ${instruction} — ${hint}`
        : `- ${name}: ${instruction}`,
    );
  }
  if (promptAppend) {
  ...
  add(`Thread ID: ${threadId}`);
  add(`Workspace ID: ${workspaceId}`);
  add(`Organization ID: ${orgId}`);
  return lines.join("\n");
}
