...
describe("chat runtime high-memory bounds", () => {
  it("keeps each selected serialized-payload phase inside its accounting envelope", () => {
    // JSON validation, checkpointing, and stream encoding may briefly retain
    // both an object/string and its encoded copy. Count every active payload
    // twice instead of assuming prompt garbage collection.
    const checkpointMutationPayloads =
      CHAT_RUNTIME_BOUNDS.sseDoBytes +
      4 * CHAT_RUNTIME_BOUNDS.checkpointBytes +
      2 *
        (CHAT_RUNTIME_BOUNDS.contextBytes +
          CHAT_RUNTIME_BOUNDS.liveMessageBytes +
          CHAT_RUNTIME_BOUNDS.systemPromptBytes +
          CHAT_RUNTIME_BOUNDS.toolSchemaBytes +
    ...
    const fileTransferPayloads =
      ...
      // Non-streamable workspace destinations are capped at one source-read
      // payload. Six copies conservatively cover raw bytes, both 4/3 base64
      // strings, and an RPC structured-clone copy; streamable 8 MiB transfers
      // use backpressured ReadableStreams instead of whole-file buffers.
      6 * CHAT_RUNTIME_BOUNDS.toolSourceReadBytes;
    const migrationPayloads =
      ...
      2 *
        (CHAT_RUNTIME_BOUNDS.legacyMigrationBytes +
          CHAT_RUNTIME_BOUNDS.snapshotBytes +
          CHAT_RUNTIME_BOUNDS.requestBytes);
    const forkImportPayloads =
      CHAT_RUNTIME_BOUNDS.sseDoBytes +
      3 * CHAT_RUNTIME_BOUNDS.historyBytes +
      2 * CHAT_RUNTIME_BOUNDS.snapshotBytes;

    for (const phase of [
      checkpointMutationPayloads,
      toolExecutionPayloads,
      fileTransferPayloads,
      migrationPayloads,
      forkImportPayloads,
    ]) {
      expect(phase).toBeLessThanOrEqual(
        CHAT_RUNTIME_BOUNDS.accountedChatPayloadEnvelopeBytes,
      );
    }
    expect(
      CHAT_RUNTIME_BOUNDS.sseWriterQueueBytes *
        CHAT_RUNTIME_BOUNDS.sseWritersPerThread,
    ).toBeLessThanOrEqual(CHAT_RUNTIME_BOUNDS.sseDoBytes);
    expect(CHAT_RUNTIME_BOUNDS.snapshotBytes).toBeLessThan(
      CHAT_RUNTIME_BOUNDS.sseWriterBytes,
    );
    expect(CHAT_RUNTIME_BOUNDS.sseWriterBytes).toBeLessThanOrEqual(
      CHAT_RUNTIME_BOUNDS.frameBytes,
    );
  });
