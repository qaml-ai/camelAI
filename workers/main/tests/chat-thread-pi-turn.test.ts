...
describe('ChatThreadDO Pi turn handling', () => {
  ...
  it('opens a UI marker and hands a fresh prompt to ai-chat', async () => {
    const order: string[] = [];
    const fake = Object.create(ChatThreadDO.prototype) as any;
    ...
    fake.readPiActiveTurn = vi.fn(() => null);
    // The marker supplies a stable UI identity; the service owns execution state.
    fake.openPiActiveTurnIfAbsent = vi.fn(() => order.push('marker'));
    fake.appendPiCoreMessagesIfMissing = vi.fn(async () => { order.push('mirror'); });
    ...
    // UI identity is established before handing the prompt to the adapter.
    ...
    expect(order.slice(0, 2)).toEqual(['marker', 'saveMessages']);
    // The attributed prompt is queued for onChatMessage's fresh path.
    ...
    expect(order).toEqual(['marker', 'saveMessages']);
  });
  ...
  it('leaves the service execution running when the UI stream disconnects', async () => {
    ...
    // Canceling a UI consumer must not cancel the independent service run.
    ...
    // Simulate a disconnected UI consumer.
    await response.body!.cancel();

    expect(fake.disposePiSession).not.toHaveBeenCalled();
    expect(fake.piSession.abort).not.toHaveBeenCalled();
    expect(fake.clearPiActiveTurnAndJournal).not.toHaveBeenCalled();
    expect(events.some((event) => event.type === 'error')).toBe(false);
  });

  it('reattaches after a DO restart without repeating an active service request', async () => {
    const fake = Object.create(ChatThreadDO.prototype) as any;
    fake.readPiActiveTurn = vi.fn(() => ({ turnId: 'active', openedAt: 1 }));
    fake.pendingPiPromptQueue = [];
    fake.messages = [{ id: 'active', role: 'assistant' }];
    fake.piSession = null;
    fake.ctx = { waitUntil: vi.fn() };
    let finish!: (value: { status: string }) => void;
    fake.continueLastTurn = vi.fn(() => new Promise(resolve => { finish = resolve; }));
    fake._retryLastUserTurn = vi.fn();
    fake.resumeActivePiTurn = vi.fn(async () => {});
    fake.handlePiTurnFailure = vi.fn();
    await ChatThreadDO.prototype['sweepOrphanedActiveTurnMarker'].call(fake);
    await ChatThreadDO.prototype['sweepOrphanedActiveTurnMarker'].call(fake);
    expect(fake.continueLastTurn).toHaveBeenCalledTimes(1);
    expect(fake._retryLastUserTurn).not.toHaveBeenCalled();
    finish({ status: 'skipped' });
    await flushWaitUntil(fake);
    expect(fake.resumeActivePiTurn).toHaveBeenCalledTimes(1);
    expect(fake.serviceReattachPending).toBe(false);
  });

  it('does not reattach while a live adapter or fresh prompt owns the UI turn', async () => {
    const fake = Object.create(ChatThreadDO.prototype) as any;
    fake.readPiActiveTurn = vi.fn(() => ({ turnId: 'active', openedAt: 1 }));
  ...
  it('does not advertise outbound channel sends in ordinary chat prompts', () => {
    ...
    expect(prompt).not.toContain('env.BROWSER');
  ...
  it('keeps per-app visibility tools and guidance in self-host mode', async () => {
    const context = {
      orgId: 'org1',
      workspaceId: 'workspace1',
      threadId: 'thread1',
    ...
    expect(prompt).not.toContain('There is no per-app public/private setting');
    ...
    expect(subagentPrompt).toContain('public publishing, schedules, visibility changes');

    const tools = ChatThreadDO.prototype['createPiToolDefinitions'].call(fake, context);
    expect(tools.map((tool: any) => tool.name)).toContain('set_app_visibility');
  });
  ...
  it('reattaches to the service request and mirrors its settled history without rerunning the model', async () => {
 const messages = [{ role: 'user', content: 'hi' }, { role: 'assistant', content: 'done' }];
 const session = { state: { messages }, resumeServiceRun: vi.fn(async () => {}), prompt: vi.fn(), continue: vi.fn() };
 const fake = createResumeFake({ piSession: session, syncAgentState: vi.fn() });
 await ChatThreadDO.prototype['resumeActivePiTurn'].call(fake);
 expect(session.resumeServiceRun).toHaveBeenCalledTimes(1);
 expect(session.prompt).not.toHaveBeenCalled();
 expect(session.continue).not.toHaveBeenCalled();
 expect(fake.appendPiCoreMessagesIfMissing).toHaveBeenCalledWith(messages);
 expect(fake.clearPiActiveTurnAndJournal).toHaveBeenCalled();
 expect(fake.piMainBaselineIndex).toBe(2);
});

  it('does not retry an interrupted service request when reattachment fails', async () => {
 const error = new Error('Run interrupted; send a new message to continue');
 const session = { state: { messages: [] }, resumeServiceRun: vi.fn(async () => { throw error; }), continue: vi.fn() };
 const fake = createResumeFake({ piSession: session });
 await expect(ChatThreadDO.prototype['resumeActivePiTurn'].call(fake)).rejects.toThrow(error);
 expect(session.continue).not.toHaveBeenCalled();
 expect(fake.appendPiCoreMessagesIfMissing).not.toHaveBeenCalled();
});
  ...
  describe('agent loop failure & recovery guards', () => {
    // --- sendRunnerCommand: UI handoff and failure boundaries ---
    ...
    it('does NOT clear or tear down the in-flight turn when a steer-path refresh throws', async () => {
      ...
      // A failed steer reports delivery failure without tearing down the live run.
      expect(fake.appendPiCoreMessagesIfMissing).not.toHaveBeenCalled();
      expect(fake.buildUserUiSkeleton).toHaveBeenCalledWith({
        rawContent: 'also rename the table',
        clientMessageId: undefined,
    ...
    it('keeps the UI marker while handing a fresh turn to ai-chat', async () => {
      const fake = Object.create(ChatThreadDO.prototype) as any;
      ...
      // The UI marker remains until the service reports completion or failure.
      expect(fake.openPiActiveTurnIfAbsent).toHaveBeenCalledTimes(1);
      expect(fake.appendPiCoreMessagesIfMissing).not.toHaveBeenCalled();
      expect(fake.saveMessages).toHaveBeenCalledTimes(1);
      expect(fake.clearPiActiveTurnAndJournal).not.toHaveBeenCalled();
