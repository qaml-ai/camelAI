...
describe("requireChatWebSocketAccess", () => {
  ...
  it.each([
    ...
    "maps %s without another OrgDO RPC",
  ...
  it("does not multiply a transient authorization RPC", async () => {
    const validateChatWebSocketAccess = vi
      .fn()
  ...
  it("returns degraded access when authorization DOs are overloaded", async () => {
    const overloaded = Object.assign(
      new Error("Durable Object is overloaded"),
      {
        overloaded: true,
        retryable: true,
      },
  ...
  it("still rejects sessions invalidated by logout", async () => {
    const { env } = buildEnv({
      userStub: {
        getSessionInvalidatedAt: vi.fn().mockResolvedValue(Date.now() + 60_000),
      },
    });
    const req = await buildRequest();
